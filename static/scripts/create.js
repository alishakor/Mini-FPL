...
// 			playerCard.querySelector("#player-position");
...
function loopThrough(data) {
	...
	for (let i = 0; i < data.length; i++) {
        const playerData = [data[i].web_name, `${data[i].now_cost / 10} M`]
        const playerString = playerData.join(' ');
		webNames.push(playerString);
	}
	return webNames;
}
...
function handleSelectPlayers() {
	...
	.then((response) => response.json())
	.then((data) => {
		const playerNames = loopThrough(data.elements);
		const playerSelect = document.querySelectorAll(".testing");
		console.log(playerSelect);
		const squadCreate = document.querySelector(".squad-create");

		squadCreate.addEventListener("click", () => {
			playerSelect.forEach((item) => {
				playerNames.forEach((player) => {
					const modal = document.createElement("option");
					modal.setAttribute("value", player);
					modal.innerText = player;
